...
def load_dwi(path):
    ...
    try:
        ...
        img0, orientation_transpose = transpose_raw2rpi(img[:, :, :, 0], orientation)
        img1, orientation_transpose = transpose_raw2rpi(img[:, :, :, 1], orientation)
        spacing = compute_spacing(affine, orientation_transpose)
        img0 = transpose_lps2dhw(img0, spacing)
        img1 = transpose_lps2dhw(img1, spacing)
        img = np.concatenate([img0, img1], axis=0)
        img = clip_by_percentile(img, 0.5, 99.5)
        return img
    except Exception:
        return None
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser('Stroke', parents=[get_args_parser()])
